...
class TelaCarro (TelaVeiculo):
    ...
    def salvar(self):
        ...
        portas = self.txtAno.text() #variável veículo recebe a classe Veiculos com as variáveis definidas na função (modelo, ano) que são os parâsmetros que ela normalmente recebe.
        if portas !="":
            portas = int(portas)

        carro = Carro(modelo, ano, portas)#variavel Carro recebe a classe Carro (modelo, Ano, portas)
